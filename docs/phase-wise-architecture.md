...
|-------|----------|------------|--------|
| **Backend** | [Streamlit Community Cloud](https://streamlit.io/cloud) (or self-hosted Streamlit Server) | **`streamlit_app.py`** at the **repository root** — main file to select in the Cloud “app path” UI. Installs from **`requirements.txt`** (root). Loads the corpus, runs filter + Phase 3, Streamlit UI. | Pin Python version and dependencies (`requirements.txt` / `pyproject.toml`) to match CI. Configure **Streamlit Secrets** for `GROQ_API_KEY`, dataset env vars (`RECOMMENDER_DATASET_*`, etc.), and any caps from Phase 4.4 — do not commit secrets. Cold starts and resource limits follow Streamlit’s hosting tier; document expected first-request latency. |
...
| 1.7 | 2026-05-01 | Root **`streamlit_app.py`** + **`requirements.txt`** for Streamlit Cloud; architecture Deployment row documents main file path. |
